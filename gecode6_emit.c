...
static void print_properties_update(FILE* out,Emit* r){
	...
	while(body){
		...
		case EMIT_BOOLDECL:
		case EMIT_INTDECL:
			fprintf(out,"\t\t%s.update(*this,_model.%s);\n",body->name.str,body->name.str);
			break;
		default:
			break;
		}
...
void gecode6_emit(FILE* out,Emit* r){
	...
	fprintf(out,"\t}\n\tModel(Model& _model):Space(_model)");
	tmp=r;
	while(tmp){
		if(tmp->type!=EMIT_PRED) fprintf(out,"\t\t/*unsupported construct*/\n");
