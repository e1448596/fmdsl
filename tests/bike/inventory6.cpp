...
#define FILL(table) table.init(sizeof(table##_data[0])/sizeof(int));for(int r=0;r<sizeof(table##_data)/sizeof(table##_data[0]);++r){table.add(IntArgs(sizeof(table##_data[0])/sizeof(int),table##_data[r]));}table.finalize()
public:
	TupleSet sprockets;
	TupleSet saddle;
	TupleSet shifter;
